...
def changing_the_direction_of_movement(dir, fi, teta):
    if abs(dir["Yz"]) > 0.9999:
        ...
        x = delsqrt * (
            dir["Yx"] * dir["Yz"] * math.cos(fi) - dir["Yy"] * math.sin(fi)
        ) + (dir["Yx"] * math.cos(teta))

        y = delsqrt * (
            dir["Yy"] * dir["Yz"] * math.cos(fi) + dir["Yx"] * math.sin(fi)
        ) + (dir["Yy"] * math.cos(teta))

        z = -math.sin(teta) * math.cos(fi) * math.sqrt(1 - math.pow(dir["Yz"], 2)) + (
            dir["Yz"] * math.cos(teta)
        )
